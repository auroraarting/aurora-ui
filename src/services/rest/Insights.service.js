...
export const getInsightsCategories = async () => {
	...
	/** A term list as a GraphQL connection: ordered the way WPGraphQL ordered
	 *  terms, with HTML entities decoded in the name. */
	const terms = (list) =>
		toConnection(
 ...
 *  call per language.
 *
 *  Nothing in this module uses it. It is exported because
 *  rest/Softwares.service.js calls a `loadTranslated` it never defines, and
 *  this is the function it means — read the note at that call site before
 *  wiring the two together, because the signatures do not match. Exporting
 *  changes nothing today: no module imports it yet. */
export async function loadTranslated(items, base, fields, pageID) {
