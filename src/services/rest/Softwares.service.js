...
/** One REST call, tagged with the caller's apiID and this module's pageID. */
...
/** One ACF button group reshaped to the GraphQL field names, or null when the
 *  group is empty. */
...
/** The `available_regions` group: the region tab and its marquee. */
...
/** The `banner` group, including its video rows. */
...
/** The `case_study` group. The selected case studies arrive expanded, so the
 *  ids are kept only for the shape GraphQL returned. */
...
/** The `keyAdvantages` group.
 *
 *  `desciption` is misspelled in ACF and was misspelled in the GraphQL
 *  response too, so the typo is reproduced deliberately - components read it. */
...
/** The `map` group: the marquee and the header logo. */
function mapMap(acf) {
...
/** The `4_step_process` group and its step rows. */
...
/** The `insights` group. `extended` adds the fields only the detail page
 *  selected, so the listing keeps the narrower shape it had. */
...
/** The `our_client` group: logos and testimonials, both arriving expanded.
 *  `withTabTitle` adds the tab title only the pages that showed one selected. */
...
/** The `why_aurora` group and its list rows. */
...
/** The /software landing page's own `banner` and `insights` groups. Returns an
 *  object either way, with null members, because the landing query always
 *  selected both. */
...
async function getCountries({ withTranslations = false } = {}) {
	...
	// KNOWN DEFECT, currently unreachable. `loadTranslated` is not defined or
	// imported in this module — it exists as a private function in
	// rest/Insights.service.js — so this line would throw a ReferenceError. It
	// never runs: getCountries() has one caller, below, which passes no
	// argument, so `withTranslations` is always false and the early return above
	// always wins. Do not call getCountries({ withTranslations: true }) until
	// this is resolved. Note also that Insights' loadTranslated expects rows
	// carrying `.translations` and is handed a Map here, whose iteration yields
	// [key, value] pairs, so importing it as-is would return empty translations
	// rather than working. Either finish it or delete the branch.
