...
export default function WebinarListing({
	...
	const optionsData = {
		...
		yearsType: Array(new Date().getFullYear() - 2024)
			.fill(null)
			.map((item, ind) => {
	...
	useEffect(() => {
		...
		if (params.size > 0) {
			...
			const filteredArr = filterItemsForWebinar(data, selecObj);
			setList(filteredArr);
			setPaginationArr(filteredArr);
			setLoading(false);
		}
				...
				<div className={`${styles.insightsItemFlex} d_f m_t_20`}>
					...
					{list?.length === 0 && !loading && (
						<p className={`${styles.nodataText} nodataText`}>
							No webinars available for this selection. Please choose a different
							option.
						</p>
					)}
				</div>
